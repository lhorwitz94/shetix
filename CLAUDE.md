...
**Ticket-CTA tiles reuse `EventCard.tsx` verbatim** (same white background, badges, price rows, "Best price" highlight as the homepage) — intentional, so the two never visually drift apart. The wrapper spans `col-span-1 row-span-3` (295px × 564px at desktop 4-col width) — same column width as a regular mosaic tile, not the double-width span an earlier version used. `row-span-3` isn't arbitrary: `row-span-2` was tried first and measurably clips a 4-market-plus-subtitle event's price rows (confirmed via `scrollHeight` vs `clientHeight` in a Playwright test, not just eyeballing it) once EventCard is squeezed into a single narrow column instead of two — re-verify with the same kind of measurement if the column count or gap sizing ever changes. A `[&>div]:h-full` wrapper class stretches EventCard's root `<div>` to fill that height (combined with its own internal `flex flex-col` + `flex-1` content section, pushing the price-row footer flush to the bottom rather than leaving dead space under shorter/single-market cards) — without editing EventCard.tsx. The purple ring around the tile is a `box-shadow` on the *wrapper*, not a border on EventCard's own root element, specifically so it doesn't fight EventCard's existing `border-gray-100` for CSS specificity.

A tradeoff worth knowing: because EventCard's fixed padding/text doesn't shrink to fit a narrower column, these tiles end up noticeably taller/more portrait than EventCard's natural landscape proportions on the homepage (~394px wide × ~270px tall there vs ~295px × 564px here). Width matches other mosaic tiles; the extra height is what unmodified EventCard content actually needs to avoid clipping at that width — it's not exactly the same aspect ratio as the homepage.
