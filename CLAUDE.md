...
The header block (badge/title/venue) uses `flex-1 justify-center` rather than pinning the price-row footer to the bottom via `mt-auto` — events with only 1-2 markets have a much shorter footer, and top-anchored header + bottom-pinned footer left a visually awkward dead gap in the middle (confirmed via a zoomed screenshot, not just measurement). Centering the header content in its available space distributes that slack more evenly instead.
