...
**Ticket-CTA tiles are a condensed card, not a literal `EventCard.tsx` reuse** — that was tried first (full EventCard reused verbatim), but at a single mosaic column's width it needed `row-span-3` (verified via `scrollHeight` vs `clientHeight` measurement — `row-span-2` measurably clipped a 4-market-plus-subtitle event's price rows) to avoid clipping, which made the tile far more portrait than a regular small tile. Went with a purpose-built compact tile instead, sized `col-span-1 row-span-1` — identical footprint to a regular small news tile (confirmed no clipping even with a deliberately long title, via the same measurement approach). It keeps EventCard's visual language (same `SPORT_STYLES` badge-color mapping, duplicated locally in `NewsMosaic.tsx` rather than exported from `EventCard.tsx`, plus a purple `box-shadow` ring) but shows only one best-price line instead of a full per-market breakdown — there isn't room for more at that size.
