...
Video items reuse the exact same `NewsItem` shape as articles (`link` = watch URL, `image` = thumbnail, `excerpt` = `''`) specifically so they flow through the *existing* `sortItems()`/filter/`buildTiles()` pipeline in `NewsMosaic.tsx` completely unchanged — the only UI change for videos is a play-icon hover overlay added to `NewsTile` (conditional on `item.contentType === 'video'`), everything else (tile shell, gradient overlay, league-badge/title/source layout, grid-flow-dense packing) is identical to articles by construction, not by coincidence. This was a deliberate integration choice to avoid touching working sort/filter/tile-building logic while adding a new content type.
