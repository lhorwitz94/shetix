...
**Ticket-CTA tiles are a condensed card, not a literal `EventCard.tsx` reuse**, shaped `col-span-2 row-span-3` (~602×564, deliberately near-square) — the largest, most distinct tile in the mosaic, chosen specifically so it doesn't blend in with either the square/wide article tiles or the tall vertical video tiles. Styling is a **white card as the dominant surface with a purple ring border** (`boxShadow: '0 0 0 3px #9966CB'` on the card itself, not a colored background with white floating inside — that was an earlier iteration, moved away from per explicit direction). Content: sport badge (with ticket-icon SVG) *inside* the white card near the top, title, venue/city/state, and up to 4 per-market price rows with "Best price" highlighting — same content set as `EventCard.tsx`, sized close to its actual proportions (`text-lg` title, `text-sm` elsewhere) since this tile has much more room than earlier smaller iterations did. `SPORT_STYLES`/`MARKET_TEXT` are duplicated locally in `NewsMosaic.tsx` (not exported from `EventCard.tsx`) so colors match exactly. The market-row fallback text is "Tickets" (not "See tix") when a market has no listed price.

The badge+title+venue block and the price-rows block are wrapped together and centered as a group via `justify-center` on the outer flex container (not `mt-auto` pinning the price-rows to the bottom) — with a fixed-height tile this large, a 1-market event's footer is much shorter than a 4-market one's, and bottom-pinning concentrates all the leftover space into one visually awkward gap between venue and price rows. Centering distributes it evenly instead. This is the same lesson learned (and previously undone, then re-learned) earlier in this tile's history — if you touch this layout again, re-check both a 1-market and a 4-market-plus-long-title event via a real screenshot, not just code review, since the "must re-fix the dead-space gap" mistake has already recurred once.

Sizing history worth knowing if this changes again: went from `col-span-1 row-span-1` (blended in too much, too small to stand out) → briefly `col-span-1 row-span-1` compact card → this square `col-span-2 row-span-3` version, in response to explicit "needs to stand out more, more square-ish" feedback. On mobile (`grid-cols-2`), `col-span-2` naturally becomes full-width — verified this still looks clean (full-width white card, not squeezed or overflowing) rather than assuming the desktop screenshot generalizes.
