...
Image extraction (`resolveImageUrl` in `lib/news.ts`) has to normalize each source's raw `<img src>`/enclosure/media URL — some feeds (The GIST) emit a site-relative Gatsby remote-file proxy path like `/_gatsby/file/<hash>/name.jpg?u=<encoded-original-url>` that 404s if hotlinked as-is from wtix's own domain; the fix pulls the real image straight out of that `u` query param. Image coverage still varies by source — some feeds simply don't expose an inline image at all, which is expected (no-image tiles render as 1x1, by design, not a bug). `vercel.json`'s cron just re-warms the cache every 2h; check your Vercel plan supports crons at that frequency (Hobby historically limits crons to once/day) before relying on it.
