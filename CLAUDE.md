...
**Ticket-CTA tiles reuse `EventCard.tsx` verbatim** (same white background, badges, price rows, "Best price" highlight as the homepage) — intentional, so the two never visually drift apart. The tile wrapper spans `col-span-2 row-span-2` (372px tall), sized to comfortably fit even a 4-market event with a subtitle without clipping (verified via a Playwright height measurement across mocked 1–4 market events). Since `EventCard.tsx` itself has no fixed height, a wrapper class of `[&>div]:h-full` stretches its root `<div>` to fill that cell — combined with `EventCard`'s own internal `flex flex-col` + `flex-1` content section, this pushes the price-row footer flush to the bottom instead of leaving dead white space under shorter cards (e.g. single-market events). If `EventCard.tsx`'s root layout ever changes, re-check that this still holds instead of assuming it does.
