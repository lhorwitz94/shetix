...
**Two-tone purple/white styling**: the outer tile (`col-span-1 row-span-1`) has a solid purple (`bg-[#9966CB]`) background; badge/title/venue/price-rows all live together in a white inner box (`div.bg-white rounded-lg`) sized to its own content, not stretched — so events with fewer markets leave purple visible below the white box, which reads as intentional framing rather than a bug. This replaced an earlier version that pinned the price-row footer to the bottom via `mt-auto` inside a single white card, which left an awkward dead gap for 1-2-market events (confirmed via a zoomed screenshot, not just measurement). The market-row fallback text is "Tickets" (not "See tix") when a market has no listed price.
