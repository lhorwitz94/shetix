...
Video items reuse the exact same `NewsItem` shape as articles (`link` = watch URL, `image` = thumbnail, `excerpt` = `''`) specifically so they flow through the *existing* `sortItems()`/filter/`buildTiles()` pipeline in `NewsMosaic.tsx` completely unchanged — everything (gradient overlay, league-badge/title/source layout, grid-flow-dense packing) is identical to articles by construction, not by coincidence. Two video-specific differences in `NewsTile`: every video tile is always `col-span-2 row-span-1` (a wide horizontal widget shape, overriding the article-only `big`-every-7th alternation) so it stands out from the square/tall article tiles, and it always shows a play-button circle overlay (not hover-only — it's the "this is a video" affordance, so it needs to be visible without interaction). The horizontal-only shape was a deliberate simplification: YouTube's public feed always reports thumbnail dimensions as a fixed 480x360 container regardless of the actual video's real orientation (verified against real feed data — identical across every entry, every channel, including likely-Shorts-style clips), so there's no reliable feed-only signal to size tiles per-video's true aspect ratio; don't try to reintroduce per-video vertical/horizontal detection without an extra lookup (e.g. YouTube's oEmbed endpoint, which does return real width/height) and a good reason to justify the added latency of one more request per video.
