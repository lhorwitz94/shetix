...
**Two-tone dark/white styling**: the outer tile (`col-span-1 row-span-1`) uses the *same dark gradient as the hero/header* (`linear-gradient(135deg, #060011 0%, #1a0638 45%, #2a0a50 55%, #060011 100%)` — same literal value as `Header.tsx`/`app/page.tsx`'s hero background, not the flat `#9966CB` brand accent used elsewhere), and is vertically centered (`justify-center` on the outer flex container). Title/venue/price-rows live in a white inner box (`div.bg-white rounded-lg`) sized to its own content, not stretched — centering means events with fewer markets split their leftover space above and below the white box rather than dumping it all at the bottom. The sport badge (with a ticket-icon SVG) is *not* inside that white box — it's `absolute top-1.5 left-1.5` on the outer (relatively-positioned) container, sitting directly on the dark background, specifically so it reads as "ticket promo for [sport]" at a glance rather than blending in with the card's own text content. `SPORT_STYLES`/`MARKET_TEXT` are duplicated locally in `NewsMosaic.tsx` (not exported from `EventCard.tsx`) so colors match exactly. The market-row fallback text is "Tickets" (not "See tix") when a market has no listed price. All of this went through multiple iterations caught via zoomed Playwright screenshots, not just code review — re-verify no clipping (`scrollHeight` vs `clientHeight`) across 1-4 market events if this layout changes again.
